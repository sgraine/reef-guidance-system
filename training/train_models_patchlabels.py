...
# `from dataloaders...` work even when running the script from
# the training/ directory or another working directory.
...
if __name__ == '__main__':
    ...
    train_dataloader, val_dataloader = loadDataValSplit(class_list, dataset_path, batch_size, augment=augment, num_workers=8)
    ...
    for count in class_counts:
        ...
        weight = 1.0 - float(count) / float(total_samples)
    ...
    for epoch in range(epochs):
        ...
        #if per_class_f1_val.mean() > best_acc:
        if acc_val > best_acc:
            best_acc = acc_val
            torch.save(model.state_dict(), os.path.join(args.output_path, f"{model_name}_best.pt"))

        # Save the lowest loss model weights
        #if val_loss < min_loss:
        #    min_loss = val_loss
        #    torch.save(model.state_dict(), os.path.join(args.output_path, f"{model_name}_loss_CKPT.pt"))
