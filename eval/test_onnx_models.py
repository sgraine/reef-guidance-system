...
def preprocess_image(image, input_shape, im_or_patch:str):
    ...
    if im_or_patch=='im':
        ...
        # convert to rgb
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)   # shape = (3040, 5312, 3)
        # print(image[0,0,:])

        # Divide the full image into a grid of patches
        grid, _, _ = img_to_grid(image,4,7)  # row, column   # list of len 28 x np.array() of shape (760, 759, 3)
        ...
        for patch in grid:
            ...
            # Change the image from HWC to CWH format (channels first)
            patch = np.transpose(patch, (2, 0, 1))  # shape = (760, 759, 3)
            # print("first pixel in first patch RGB:")
            # print(patch[:,0,0]) 
            ...
            patch = np.transpose(patch_crop, (1, 2, 0))  # Convert (C, W, H) to (H, W, C)    # shape = (760, 758, 3)
            # print(patch[0,0,:]) 
            # print(np.shape(patch))

            # Resize to 256,256
            patch = np.array(Image.fromarray(patch).resize((256,256)))   # shape = (256, 256, 3)
            # print("after resize:")
            # print(patch[0,0,:]) 
            # print(np.shape(patch))
            ...
            patch = patch.astype(np.float32) / 255.0   # shape = (256, 256, 3)
            # print("after /255:")
            # print(patch[0,0,:]) 
            # print(np.shape(patch))
            

            patch = np.transpose(patch, (2, 0, 1))   # Convert (H, W, C) to (C, W, H)  # shape = (3, 256, 256)
...
def save_patch_comparison(i, np_patch, torch_patch, diff_patch, output_dir):
    np_img = (np_patch.transpose(1, 2, 0) * 255).astype(np.uint8)
    torch_img = (torch_patch.transpose(1, 2, 0) * 255).astype(np.uint8)
    diff_img = (diff_patch.transpose(1, 2, 0) * 255).astype(np.uint8)
...
if __name__ == '__main__':
    ...
    save_folder = 'outputs/saved_inferences/model-1745448701CKPT-rosbag-inferences-onnx'
    output_dir = 'outputs/onnx-patch-debug'
    os.makedirs(output_dir, exist_ok=True)
    
    ################## ONNX model WITHOUT preprocessing ####################
    print("################## ONNX model WITHOUT preprocessing ####################")
    # Load the ONNX model
    onnx_model_path = "outputs/models/onnx/Mobilenet-wholeimage-1-3-1328-760.onnx"
    ...
    image = cv2.imread(image_path)    # shape = (3040, 5312, 3)
    print(np.shape(image))
    print(image[0,0,:])
    # input_np = preprocess_image(image, (256,256), 'patch')
    input_np = preprocess_image(image, (1328,760), 'im')

    # np_patch = input_np[0,:,:,:].astype(np.float32)
    ...
    class_pred = ort_session.run(output_names, {input_name: input_np})[0]
    ...
    #     ratio = outputs[1]          # Predicted class (argmax)
    #     deploy = outputs[2]         # Ratio of top two softmax values
    #     torch_patch = outputs[3][0,:,:,:]
    #     # print(outputs[3][0])
    #     # print(outputs[4][0])
    #     # print(outputs[5][0])
    #     # # patches_2 = patches
        
    #     print(outputs[4][0,:,0,0])   # shape = (28, 3, 760, 758)
    #     print(outputs[5][0,:,0,0])   # shape = (28, 3, 256, 256)
    #     print(outputs[3][0,:,0,0])   # shape = (28, 3, 256, 256)
    #     # print(outputs[4][0])
    ...
    #     # diff = np.abs(np_patch - torch_patch)
    #     # # print(difference)
    ...
    # # save_patch_comparison(0, np_patch, torch_patch, diff, output_dir)