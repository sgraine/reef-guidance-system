...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Script for extracting coral patches from images using CLIP.')
...
if __name__ == '__main__':
    ...
    dataset_save_path = args.output_data #"CleanData/Training - CLIPPatchLabelling/Site 5/Site 5-coral"

    prev_class_list = ["Coral"]
    ...
    # Prompts for the coral images: we need to filter out water and dark patches
    text = clip.tokenize(["a photo of sand", "a really dark photo",
                        "a photo of water", "a photo of blue water","a photo of black",
                        "a photo of a coral reef", "a photo containing coral"]).to(device) 
    ...
    for inputs, image_label, whole_image, image_name in tqdm(dataloaders['train']):
        ...
        for patch in inputs:
            ...
            no_dep_indices = slice(0, -2)       # filtered category
            coral_indices = slice(-2, None)     # coral category
            ...
            scaled_probs = scale_probability(torch.tensor(add_probs), 1, scale_factor)
            ...
            if bag_int == 0:
                ...
                coral += 1
                save_path = os.path.join(dataset_save_path, prev_class_list[0])+"/"+image_name[0]+"_"+str(i)+".png"
                pil_img = Image.fromarray(np.squeeze(patch)).save(save_path)     

            i += 1
            total += 1           

    print("Out of a total", total, "patches,", no_dep, "were filtered and there were", coral, "coral patches.")
