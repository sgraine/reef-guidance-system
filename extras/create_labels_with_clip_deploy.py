...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Script to create labels for deploy images using CLIP.')
...
if __name__ == '__main__':
    ...
    dataset_save_path = args.output_data #"CleanData/Training - CLIPPatchLabelling/Site 5/Site 5-deploy"

    prev_class_list = ["Deploy"]
    ...
    # Prompts for deploy images: we need to filter out coral patches as well as water and dark patches
    text = clip.tokenize(["a really dark photo","a photo of water", "a photo of blue water","a photo of black",
                            "a photo of a coral reef", "a photo which contains some coral",
                            "a photo of the seafloor", "a photo of sand"]).to(device) 
    ...
    for inputs, image_label, whole_image, image_name in tqdm(dataloaders['train']):
        ...
        for patch in inputs:
            ...
            no_dep_indices = slice(0, 4)   # filtered category
            coral_indices = slice(4, 6)   # save category
            dep_indices = slice(-2, None)   # save category

            # Compute the summed probabilities for each category in a single line each
            no_dep_sum = probs[0][no_dep_indices].sum()
            coral_sum = probs[0][coral_indices].sum()
            dep_sum = probs[0][dep_indices].sum()

            # Add multiplier to coral:
            add_probs = [no_dep_sum, coral_sum, dep_sum]
            scale_factor = 2.6  # (for deploy)

            scaled_probs = scale_probability(torch.tensor(add_probs), 2, scale_factor)
            ...
            if bag_int == 0:
                ...
                save_path = os.path.join(dataset_save_path, class_list[1])+"/"+image_name[0]+"_"+str(i)+".png"
                pil_img = Image.fromarray(np.squeeze(patch)).save(save_path)     
            else:
                dep += 1
    ...
    print("Out of a total", total, " patches,", no_dep, "were filtered,", coral, "were reclassified as coral, and there were", dep, "deploy patches.")
